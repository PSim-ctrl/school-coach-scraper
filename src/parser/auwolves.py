...
class Parser():
    ...
    def get_coaches_url(self, *args, **kwargs):
        try:
            ...
            urls = []
            h3_tag = soup.find('h3', text="Ice Hockey, Women's") or soup.find('h3', text="Men's Ice Hockey Coaching Staff")
            if h3_tag:
                ...
                tr_tags = soup.select('tr[data-category-id="53"]')
                for tr_tag in tr_tags:
                    if tr_tag.has_attr('data-member-id'):
                        tags = tr_tag.select('a')
                        for tag in tags:
                            if tag and tag.has_attr('class'):
                                urls.append(f"{self.base_url}"+tag['href'])
            return urls
        except Exception as e:
    ...
    def get_name(self, soup):
        try:
            f_name = None
            l_name = None
            tag = soup.select_one('.sidearm-coach-bio-name') or soup.select_one('span.sidearm-staff-member-bio-name')
            if tag:
